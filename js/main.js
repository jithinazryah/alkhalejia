...
//
    ...
    $('#dailyentrydetails-tax_id').change(function () {
        TaxAmount();
        DailyEntryTotal();
    });
    ...
    function TaxAmount() {
        ...
        var tax = $('#dailyentrydetails-tax_id').val();
        ...
        $.ajax({
            ...
            cache: false,
            data: {tax: tax},
            url: homeUrl + 'purchase/daily-entry/tax',
            success: function (data) {
                if (data != '') {
                    var tax_amount = (total * data) / 100;
                    var tax_amount = tax_amount.toFixed(2);
                    $('#dailyentrydetails-tax_amount').val(tax_amount);
    ...
    function DailyEntryTotal() {
        var subtotal = 0;
        var rate = $("#dailyentrydetails-rate").val();
        var unit = $("#dailyentrydetails-net_weight").val();
        var total = rate * unit;
        var tax = $('#dailyentrydetails-tax_amount').val();
        if (tax == '') {
            tax = 0;
        }
        if (total != '')
        ...
        $('#dailyentrydetails-total').val(subtotal.toFixed(2));
    }
