        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            foreach ($arr as $val) {
                ...
                if (isset($val['total']) && $val['total'] != '') {
                    ...
                    $total = $daily_entry->total;
                }
                if (isset($val['amount_paid']) && $val['amount_paid'] != '') {
                    $daily_entry->amount_paid = $val['amount_paid'];
                    $amount_paid = $daily_entry->amount_paid;
                }
                if (isset($val['description']) && $val['description'] != '') {
                    $daily_entry->description = $val['description'];
                }
                $daily_entry->due_amount = $total - $amount_paid;
                $daily_entry->received_date = date("Y-m-d h:i", strtotime($daily_entry->received_date));
...
//            $model = new DailyEntry();
    ...
    public function SavePurchaseDetails($model, $data) {
        ...
        if (isset($_POST['creates']) && $_POST['creates'] != '') {
            ...
            $i = 0;
            foreach ($_POST['creates']['amount_paid'] as $val) {
                $arrs[$i]['amount_paid'] = $val;
                $i++;
            }
            $i = 0;
