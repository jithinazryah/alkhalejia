...
<div class="contacts-form form-inline">
    ...
    <div class="row">
        <div class='col-md-4 col-sm-6 col-xs-12 left_padd'>
            <?= $form->field($model, 'service')->dropDownList(['1' => 'Material purchase(Crusher)'], ['readonly' => TRUE]) ?>
        </div>
    ...
    <div class="row">
        ...
        <div class='col-md-12 col-sm-12 col-xs-12'>
            <div class="form-group">
                <?= Html::submitButton($model->isNewRecord ? 'Create' : 'Update', ['id' => 'add_supplier', 'class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary', 'style' => 'margin-top: 18px; height: 36px; width:100px;float:right;']) ?>
            </div>
        </div>
    </div>
        ...
        if (valid()) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if ($data.con === "1") {
                        $('#dailyentry-supplier').append($('<option value="' + $data.id + '" selected="selected" >' + $data.name + '</option>'));
