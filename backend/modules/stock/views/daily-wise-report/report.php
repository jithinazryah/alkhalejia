                    ...
                    <div class="row" style="margin-left: 0px;">
                        ...
                        <div class="col-md-9">
                            <div class="col-md-3">
                                ...
                                if (isset($model->DOC)) {
                                    $date = $model->DOC;
                                    $model->DOC = date('d-m-Y', strtotime($model->DOC));
                                } else {
                                    $model->DOC = date('d-m-Y');
                                }
                                ?>
                            ...
                            $result = '';
                            $i = 0;
                            if (!empty($model->material_id) && $model->material_id != '') {
                                foreach ($model->material_id as $val) {
                                    if ($i != 0) {
                                        $result .= ',';
                                    }
                                    $result .= $val;
                        ...
                        <div class="col-md-3">
                            <div class="sales-invoice-master-search" style="margin-right: 15px;float: right;">
                                ...
                                <?= Html::beginForm(['daily-wise-report/exports'], 'post', ['target' => 'print_popup', 'id' => "epda-form", 'style' => 'margin-bottom: 0px;']) ?>
                                <input type="hidden" value="<?= $model->DOC ?>" name="report_date"/>
                                <input type="hidden" value="<?= $result ?>" name="material_id"/>
                        ...
                        <table class='table-responsive table table-small-font table-bordered table-striped'>
                            ...
                            foreach ($contacts as $crusher) {
                                ...
                                foreach ($model->material_id as $material_trip) {
                                    $materials_entry = DailyEntry::find()->where(['material' => $material_trip, 'supplier' => $crusher->id, 'DOC' => $model->DOC])->all();
                            ...
                            ?>
                            <tr>
                                <td><b><?= date('d-m-Y', strtotime($previous_date)) ?></b></td>
                                <?php
                                $previous_stock = 0;
                            ...
                            <tr>
                                <td><b><?= date('d-m-Y', strtotime($model->DOC)) ?></b></td>
                                <?php
                                $stock = 0;
                                $total_stock_net = 0;
